...
module.exports = {
  ...
  async single(course_id) {
    const sql = `select * from course co join (select course_id, avg(rating) as rating, count(student_id) as numberStu, count(if(rating > 0, 1, null)) as rateStu from stu_registerlist group by course_id having course_id = ${course_id}) ra on co.course_id = ra.course_id join course_spec cs on co.spec = cs.spec_id  join teacher te on co.teacher_id = te.teacher_id join course_type ct on co.type=ct.type_id join course_detail cd on co.course_id = cd.course_id;`;
    const [rows, fields] = await db.load(sql);
    if (rows.length === 0)
      return null;
