...
module.exports = {
  ...
  async pageSearchResultSortRelevance(search, offset) {
    const sql = `select distinct co.course_id, co.disablez, co.name, if(DATEDIFF(curdate(), co.publish_day) < 14, 1, null) as newest,  (co.price - co.price*co.discount/100) as newprice, co.link_ava_course, co.discount, co.view_number, co.price, ra.numberStu, ra.rateStu, ra.rating, cs.spec_name, ct.type_name, te.fname, te.lname,  DATEDIFF(curdate(), co.publish_day) as day_ago from course co left  join (select course_id, avg(rating) as rating, count(student_id) as numberStu, count(if(rating > 0, 1, null)) as rateStu from stu_registerlist group by course_id) ra on co.course_id = ra.course_id join course_spec cs on co.spec = cs.spec_id join teacher te on co.teacher_id = te.teacher_id join course_type ct on co.type=ct.type_id join course_chapter cc on co.course_id = cc.course_id join course_detail cd on co.course_id = cd.course_id where (match(co.name) against('CSS') or match(cc.chap_name) against('CSS') or match(cd.short_info, cd.full_info) against('CSS') or match(cs.spec_name) against('CSS') or match(ct.type_name) against('CSS') or match(te.fname, te.fname) against('CSS')) and co.disablez = 0 order by (match(co.name) against('CSS') + match(cc.chap_name) against('CSS') + match(cd.short_info, cd.full_info) against('CSS') +  match(cs.spec_name) against('CSS') + match(ct.type_name) against('CSS') + match(te.fname, te.fname) against('CSS')) DESC limit ${paginate.limit} offset ${offset};`;
    const [rows, fields] = await db.load(sql);
    return rows;
  },
